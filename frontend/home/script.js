...
document.addEventListener("DOMContentLoaded", async () => {
    try {
        ...
        updateEmailCounts();
        showEmailCountCheckbox.checked = true; // Imposta "Mostra conteggio email" come checked
        toggleEmailCountVisibility(true); // Mostra i conteggi di default
        filterEmails("inbox");
        await loadEvents();
...
async function loadEmailsToPage() {
    try {
        ...
        console.log("Email ricevute:", emails.length, emails.map((e) => ({ id: e.id, categories: e.categories })));
        renderEmails(emails);
...
function filterEmails(category, searchTerm = "") {
    ...
    if (category !== "calendar") {
        document.querySelectorAll(".email-item").forEach((item) => {
            const categories = item.getAttribute("data-category").split(" ").filter((c) => c);
...
deleteBtn.addEventListener("click", async () => {
    if (currentEmail) {
        ...
        let categories = currentEmail.getAttribute("data-category").split(" ").filter((c) => c);
...
aiReplyBtn.addEventListener("click", async () => {
    ...
    try {
        ...
        const data = await response.json();
        if (!response.ok) {
            console.error("Errore risposta /api/ai/gemini-reply:", data);
...
async function classifyEmails(emails) {
    try {
        ...
        if (!res.ok) {
            ...
            throw new Error(err.message || "Errore categorizzazione email");
        }

        return await res.json();
    } catch (err) {
        console.error("Errore classifyEmails:", err);
        throw err;
    }
}
...
async function generateReply(body) {
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(err.message || "Errore generazione risposta AI");
        }

        const data = await response.json();
        return data.reply;
    } catch (err) {
        console.error("Errore generateReply:", err);
        throw err;
    }
}