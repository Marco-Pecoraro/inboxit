...
async function renderEmails(emails) {
    try {
        ...
        for (const email of emails) {
            ...
            let previewText = email.body ? email.body.replace(/<[^>]+>/g, '').slice(0, 18) : "(Nessun contenuto)";
            if (previewText.length < 18) {
                previewText = previewText.padEnd(18, ' ');
            } else if (email.body && email.body.replace(/<[^>]+>/g, '').length > 18) {
                previewText = previewText.slice(0, 15) + '...';
            }
...
function toggleModal(modal, show) {
    try {
        if (modal && overlay) {
            if (show) {
                sidebarState = {
                    left: document.querySelector('.left-rectangle').classList.contains('active'),
                    right: document.querySelector('.right-rectangle').classList.contains('active')
                };
            }
            ...
            if (!show) {
                document.querySelector('.left-rectangle').classList.toggle('active', sidebarState.left);
                document.querySelector('.right-rectangle').classList.toggle('active', sidebarState.right);
            }
        }
    } catch (err) {
        console.error(err);
...
function attachSidebarToggleListeners() {
    try {
        if (leftToggle) leftToggle.addEventListener("click", () => {
            ...
            overlay.classList.toggle('active', document.querySelector('.left-rectangle').classList.contains('active') || document.querySelector('.right-rectangle').classList.contains('active'));
